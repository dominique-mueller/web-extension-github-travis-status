import { TravisCiBuild, TravisCiJob, TravisCiStage } from 'background/travis-ci/travis-ci.interfaces';
...
export class TravisCiStatus {
  ...
  constructor(statusItemElement: HTMLDivElement) {
    ...
    // Get URLs
    const [urlWithoutQueryParams, queryParams]: Array<string> = (<HTMLAnchorElement>(
      this.statusItemElement.querySelector('a.status-actions[href^="https://travis-ci.org/"]')
    )).href.split('?');
  ...
  public renderDetailedTravisCiStatus(build: TravisCiBuild): void {
    ...
    this.activeStatusDetailsNode = this.statusItemElement.parentElement.insertBefore(
      statusDetailsElement,
      this.statusItemElement.nextElementSibling,
    );
  }
  ...
  public fixMergeStatusCheckToggle(): void {
    ...
    const mergeStatusListHeight: number = Array.from(mergeStatusListElement.children).reduce(
      (height: number, mergeStatusItem: HTMLDivElement) => {
  ...
  private removeMillisecondsFromIsoDate(isoDate: string): string {
    return isoDate.indexOf('.') == -1 ? isoDate : `${isoDate.split('.')[0]}Z`;
  }
